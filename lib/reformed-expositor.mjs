...
export async function handleTranscribe(req, res) {
  ...
  try {
    ...
    const language = mapTranscriptionLanguage(body.language);

    if (!openai) {
      return sendJson(res, 503, { error: 'OPENAI_API_KEY is not configured yet.' });
    }
    ...
    if (!audioBuffer.length) {
      return sendJson(res, 400, { error: 'Audio data is empty.' });
    }

    const audioFile = await OpenAI.toFile(
      audioBuffer,
      `composer-recording.${audioExtensionForMime(mimeType)}`,
      { type: mimeType },
    );
    ...
    const transcription = await openai.audio.transcriptions.create({
      ...
      ...(language ? { language } : {}),
      prompt:
        'Bible, Scripture, expository, sermon, devotion, MacArthur, Grace to You, Puritan, Reformed, Calvin, Luther, Spurgeon, Lloyd-Jones, Sproul, Beeke, justification, sanctification, dispensational, premillennial.',
    });
...
function mapTranscriptionLanguage(value) {
  const normalized = normalizeConversationLanguage(value);
  return ['en', 'ar', 'fr', 'pt', 'sw', 'rw', 'zh', 'ko', 'ja', 'hi'].includes(normalized)
    ? normalized
    : undefined;
}
...
function buildInstructions({ mode, audience, language, libraryState, message }) {
  ...
  const base = [
    ...
    'For substantive answers, open with a short orienting heading or thesis, then move through clearly labeled sections.',
    'Within each section, give the explanation first, then weave in the supporting citation or quotation naturally.',
    ...
    'When more than one strong source is available, identify the author or ministry clearly, quote briefly, then explain the significance in plain language.',
    'Do not stack quotations without explanation. Use quotation, attribution, and then concise interpretation.',
    ...
    'If the user asks to summarize, preserve the same sources and quotations in compressed form rather than replacing them with unsupported prose.',
    'If the user asks to expand, retrieve more approved sources where possible and widen the explanation without changing the doctrinal stream.',
...
function buildLocalMacArthurFirstInstructions({ audience, language }) {
  ...
  return [
    ...
    'For substantive answers, organize the response with a short heading or thesis and clearly labeled movements.',
    ...
    'Give the quotation with attribution and then explain it plainly instead of dropping quotations without explanation.',
